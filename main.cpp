...
int main(int argc, char* argv[]) {
    ...
    //             auto[B, score] = test_on_all_k(G, 2000);
    ...
    std::string input_dir = "student_inputs/";
    std::string input_ext = ".in";
    std::string output_dir = "targeted/";
    std::string output_ext = ".out";
    #pragma omp parallel
    {
        #pragma omp for
        for(int32_t i = 0; i < len; i ++){
            std::string curr = to_improve[i];
            graph_t *G = read_input((input_dir + curr + input_ext).c_str());
            auto[B, score] = test_on_all_k(G, 5000);
            printf("FINAL SCORE OF %f\n", score);
            write_output(B, G->num_nodes, (output_dir + curr + output_ext).c_str());
            free_graph(G);
            free(B);
        }
    }
